...
class EDSR4(nn.Module):
    ...
    def __init__(self):
        ...
        for i in range(0,self.B*2):
            w=nn.Parameter(torch.ones(256,256,3,3))
            w=torch.nn.init.kaiming_normal_(w)
            b=nn.Parameter(torch.zeros(256))
            self.vars.append(w)
            self.vars.append(b)
        #64，65
        w64=nn.Parameter(torch.ones(256,3,3,3))
        w64=torch.nn.init.kaiming_normal_(w64)
        b64=nn.Parameter(torch.zeros(256))
        self.vars.append(w64)
        self.vars.append(b64)

        #66.67
        w65=nn.Parameter(torch.ones(64,256,3,3))
