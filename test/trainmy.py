    ...
    for epoch in range(args.epoches):
        ...
        if (epoch+1)%1==0:
            for i in range(24):
                ...
                viz.images(show_pred, nrow=args.qry, win='pred:{}'.format(i), opts=dict(title='{%.2f}/{%d}'%(mm,i)))
                viz.images(show_gt, nrow=args.qry, win='true:{}'.format(i), opts=dict(title='true value{}'.format(i)))
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--train', type=int, help='train number', default=3000)
    argparser.add_argument('--test', type=int, help='test number', default=100)
    ...
    argparser.add_argument('--task_lr', type=float, help='task-level inner update learning rate', default=1e-3)
    argparser.add_argument('--update_step', type=int, help='task-level inner update steps', default=1)

    #下面两个相加必须为已有因子的总数5r

    argparser.add_argument('--env', type=str, help='visdom windows title', default='bx')
